...
def simplify_pubmed_query(llm, scientist_question: str) -> str:
...
# Ensure the output is less than 10 words.
...
# Verbose Query: Has there been any significant progress in Alzheimer's disease treatment using monoclonal antibodies in the last five years?
# Output: 
# Alzheimer's disease monoclonal antibodies treatment progress

# Example 2:
# Verbose Query: What are the latest findings on the impact of climate change on the incidence of vector-borne diseases in tropical regions?
# output: 
# Climate change and vector-borne diseases in tropics

# This is the iput query:
# {question}

# Only decide to simplify the user's question if it is verbose. If it is already simple enough, just return the original user question.
# Ensure the output just 1 sentence less than 10 words.
# Only output the simplified query, no other comments needed.
# """)


pubmed_query_simplification_prompt = PromptTemplate.from_template("""
You are an expert in biomedical search queries. Your task is to simplify verbose user queries into concise, effective PubMed search queries. Focus on preserving the essential scientific or medical elements.

Examples:

Example 1:
Verbose Query: Has there been any significant progress in Alzheimer's disease treatment using monoclonal antibodies in the last five years?
Simplified: Alzheimer's disease monoclonal antibodies treatment progress

Example 2:
Verbose Query: What are the latest findings on the impact of climate change on the incidence of vector-borne diseases in tropical regions?
Simplified: Climate change and vector-borne diseases in tropics

Input query: {question}

Instructions:
- Simplify the user's question only if verbose.
- If already concise, return the original query.
- Limit the simplified query to one sentence, under 10 words.
- Only output the simplified query, no additional comments.
""")



def clean_text_biored(text):
    # Step 1: Find and remove everything between @ and $
    cleaned_text = re.sub(r'@\S+?\$', '', text)
    
    # Step 2: Replace multiple spaces with a single space
    cleaned_text = re.sub(r'\s{2,}', ' ', cleaned_text)
    
    return cleaned_text.strip()  # Strip leading/trailing spaces if any

def clean_text_semeval(text):
